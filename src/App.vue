...
import type { Provider, SyncHealth, SyncSourceConfig, Usage, UsageAccuracy, UsageSourceKind } from './types'
const page = ref<'command'|'dashboard'|'prompts'|'arena'|'accounts'|'reports'|'settings'>('command')
...
const { mode: globalMode } = useChartPreferences()
const {
  theme: appTheme,
  themeId: appThemeId,
  liquidTone,
...
let unlistenLocalSourceChanged:(()=>void)|undefined
...
onMounted(()=>{
 ...
 void listen<string>('dashboard-selection',event=>{selectedDashboard.value=event.payload}).then(unlisten=>{unlistenSelection=unlisten})
  void listen<FloatingConfigV3>('floating-config',event=>{const value=event.payload?.version===3?event.payload:loadFloatingConfigV3(floatingIds);floatingConfig.value=value;floatingCollapsed.value=value.mode==='capsule';saveFloatingConfigV3(value);if(floatingMode){void applyFloatingModeSize();void setFloatingInteraction(value.interaction);void reportFloatingSurfaces()}}).then(unlisten=>{unlistenFloatingConfig=unlisten})
 ...
 void listen<{source_id:string}>('local-source-changed',event=>{if(event.payload?.source_id)void syncReliabilitySource(event.payload.source_id,true)}).then(unlisten=>{unlistenLocalSourceChanged=unlisten})
 ...
 void listen<ProxyTrafficEvent>('proxy-request-started',event=>{
  ...
  timer=window.setInterval(()=>{
    now.value=Date.now()
    if (!isSyncing.value && now.value-lastFullSync >= refreshIntervalMs) void syncCurrentView()
  ...
  onboardingOpen.value=!floatingMode&&localStorage.getItem('token-manager-onboarding-v011')!=='done'
...
const proxyConnectionDetail=computed(()=>proxyRecentlyActive.value&&lastProxyTraffic.value
 ...
 ? autoConnectStatus.value||'已把常见 Code / SDK 与 Claude Code 指向本机代理；重启正在运行的调用工具后生效。'
...
const dashboardUsage=computed(()=>selectedDashboard.value==='__all__'?combinedUsage.value:selectedDashboard.value==='__codex__'?[]:selectedDashboard.value==='__claude__'?claudeUsage.value:usage.value.filter(item=>item.model===selectedDashboardDef.value?.model&&item.provider===selectedDashboardDef.value?.provider))
...
const balanceStatusText=computed(()=>isDeepSeekDashboard.value?(dashboardBalance.value?`官方余额 · ${dashboardBalance.value.currency}`:balanceError.value||'等待官方余额接口同步'):isOpenCodeGoDashboard.value?'套餐精确用量请查看 OpenCode 控制台':selectedDashboardDef.value?.provider==='OpenAI'||selectedDashboardDef.value?.provider==='Anthropic'?'模型 API Key 不含组织账单权限':selectedDashboardDef.value?.provider==='Google Gemini'?'需单独配置 Google Cloud Billing IAM':selectedDashboardDef.value?.provider==='自定义 OpenAI 兼容'?'由中转服务决定，当前使用代理统计':'需厂商云账户签名或账单导入')
...
const isOpenCodeGoDashboard=computed(()=>selectedDashboardDef.value?.provider==='OpenCode Go')
...
const availableModels=computed(()=>{
 const configured=savedAccounts.value.flatMap(account=>(accountModels.value[account.id]?.length?accountModels.value[account.id]:providerModels[account.provider]||['自定义模型']).map(model=>({key:`${account.id}::${model}`,accountId:account.id,provider:account.provider,model,label:`${account.name} · ${model}`})))
 const existing=new Set(configured.map(item=>`${item.provider}::${item.model}`))
 const local=[...new Set(usage.value.filter(item=>item.provider==='OpenCode Go'&&item.id.startsWith('opencode-json:')).map(item=>item.model))].filter(model=>!existing.has(`OpenCode Go::${model}`)).map(model=>({key:`opencode-local::${model}`,accountId:'opencode-local',provider:'OpenCode Go',model,label:`OpenCode 本地 JSON · ${model}`}))
...
const todayCalls=computed(()=>todayCodexTurns.value+combinedUsage.value.filter(item=>localDayKey(new Date(item.at))===localDayKey(new Date(now.value))).length)
...
type FloatingDashboardKind='all'|'codex'|'claude'|'model'
...
function floatingUsageFor(key:string){if(key==='__all__')return combinedUsage.value;if(key==='__claude__')return claudeUsage.value;if(key==='__codex__')return[];const target=modelDashboards.value.find(item=>item.key===key);return target?usage.value.filter(item=>item.provider===target.provider&&item.model===target.model):[]}
...
const floatingDashboardRows=computed<FloatingDashboardRow[]>(()=>{
 ...
 return definitions.map((item,index)=>{const source=floatingUsageFor(item.key);const calls=item.key==='__codex__'?todayCodexTurns.value:source.filter(event=>localDayKey(new Date(event.at))===localDayKey(new Date(now.value))).length;const ringPercent=item.key==='__codex__'?remaining7d.value:Math.min(100,Math.round(tokens[index]/max*100));const subtitle=item.key==='__codex__'?`${codex7dIsReported.value?'客户端额度':'本地估算'} · 7 天`:item.key==='__claude__'?'Claude Code 本地日志':item.key==='__all__'?`${activeModelCount.value} 个今日活跃模型`:`${item.provider} · ${calls} 次`;return{...item,todayTokens:tokens[index],calls,ringPercent,ringLabel:item.key==='__codex__'?`${remaining7d.value}%`:compactFloatingNumber(tokens[index]),subtitle}})
})
function floatingSeriesFor(key:string,metric:FloatingMetric){
 ...
 else for(const item of floatingUsageFor(key)){const bucket=map.get(localDayKey(new Date(item.at)));if(bucket){bucket.tokens+=item.input+item.output;bucket.calls++;bucket.cached+=item.cached;bucket.cost+=item.cost}}
...
const floatingDashboardViews=computed<FloatingDashboardView[]>(()=>floatingDashboardRows.value.map(row=>{
 ...
 const dataSource=row.kind==='codex'?(codex7dIsReported.value?'Codex 客户端 rate_limits':'个人预算估算'):row.kind==='claude'?'Claude Code 本地日志':row.kind==='all'?'主程序本地统一统计':balance?'官方账户余额 + 本地代理':'本地代理 / 日志统计'
 const balanceText=row.kind==='codex'?row.ringLabel:balance?`¥${balance.total_balance.toFixed(2)}`:row.todayTokens?'今日 Token 占比':'暂无数据'
 const ringCaption=row.kind==='codex'?(codex7dIsReported.value?'7 天客户端额度':'7 天个人估算'):'今日 Token 占比'
 return{...row,weekCost:weekCostValue,cacheRate,ringCaption,dataSource,balanceText,series:{tokens:floatingSeriesFor(row.key,'tokens'),calls:floatingSeriesFor(row.key,'calls'),cached:floatingSeriesFor(row.key,'cached'),cost:floatingSeriesFor(row.key,'cost')}}
}))
...
async function toggleFloatingDashboard(key:string){
  ...
  selectDashboard(key)
  floatingConfig.value.expandedKeys=shouldExpand?[...floatingConfig.value.expandedKeys,key]:floatingConfig.value.expandedKeys.filter(item=>item!==key)
...
function persistFloatingConfig(){const value:FloatingConfigV3={...floatingConfig.value,version:3,enabled:[...floatingConfig.value.enabled],order:[...floatingConfig.value.order],mini:[...floatingConfig.value.mini] as [FloatingModuleId,FloatingModuleId],expandedKeys:[...floatingConfig.value.expandedKeys],sizes:structuredClone(floatingConfig.value.sizes),position:floatingConfig.value.position?{...floatingConfig.value.position}:null};try{saveFloatingConfigV3(value)}catch{}void emit('floating-config',value)}
...
async function setFloatingMode(mode:FloatingMode){floatingConfig.value.mode=mode;floatingCollapsed.value=mode==='capsule';if(floatingMode)await applyFloatingModeSize();persistFloatingConfig();if(floatingMode){await nextTick();void reportFloatingSurfaces()}}
 async function rememberFloatingSize(){if(!floatingMode)return;try{const window=getCurrentWindow();const [size,scale]=await Promise.all([window.innerSize(),window.scaleFactor()]);floatingConfig.value.sizes[floatingConfig.value.mode]={width:Math.max(300,Math.round(size.width/scale)),height:Math.max(96,Math.round(size.height/scale))};try{saveFloatingConfigV3(floatingConfig.value)}catch{}}catch{}}
async function applyFloatingModeSize(){if(!floatingMode)return;const mode=floatingConfig.value.mode==='capsule'?'capsule':'compact';const size=floatingConfig.value.sizes[mode];const radius=mode==='capsule'?28:24;try{await invoke('set_floating_window_mode',{mode,radius,width:size.width,height:size.height,alwaysOnTop:floatingConfig.value.alwaysOnTop,snapToEdges:floatingConfig.value.snapToEdges})}catch{try{const window=getCurrentWindow();await window.setSize(new LogicalSize(size.width,size.height));await window.setAlwaysOnTop(floatingConfig.value.alwaysOnTop)}catch{}}}
...
async function snapFloatingToMonitor(){if(!floatingMode||!floatingConfig.value.snapToEdges)return;try{const current=getCurrentWindow();const monitor=await currentMonitor();if(!monitor)return;const [position,size,scale]=await Promise.all([current.outerPosition(),current.outerSize(),current.scaleFactor()]);const area=monitor.workArea;const threshold=Math.round(14*scale);let x=position.x;let y=position.y;const left=area.position.x;const top=area.position.y;const right=left+area.size.width-size.width;const bottom=top+area.size.height-size.height;if(Math.abs(x-left)<=threshold)x=left;else if(Math.abs(x-right)<=threshold)x=right;if(Math.abs(y-top)<=threshold)y=top;else if(Math.abs(y-bottom)<=threshold)y=bottom;if(x!==position.x||y!==position.y)await current.setPosition(new PhysicalPosition(x,y))}catch{}}
 async function initializeFloatingWindow(){if(!floatingMode)return;const window=getCurrentWindow();if(floatingConfig.value.position)try{await window.setPosition(new PhysicalPosition(floatingConfig.value.position.x,floatingConfig.value.position.y));if(!await currentMonitor())await window.center()}catch{await window.center().catch(()=>{})}await applyFloatingModeSize();if(await window.isVisible())await configureFloatingRenderer();await refreshFloatingRenderStatus();await setFloatingInteraction(floatingConfig.value.interaction);await reportFloatingSurfaces();try{unlistenFloatingResize=await window.onResized(async event=>{const scale=await window.scaleFactor();const mode=floatingConfig.value.mode==='capsule'?'capsule':'compact';floatingConfig.value.sizes[mode]=mode==='capsule'?{...defaultFloatingSizes.capsule}:{width:Math.max(300,Math.round(event.payload.width/scale)),height:Math.max(96,Math.round(event.payload.height/scale))};persistFloatingConfig();await invoke('set_floating_window_mode',{mode,radius:mode==='capsule'?28:24,alwaysOnTop:floatingConfig.value.alwaysOnTop,snapToEdges:floatingConfig.value.snapToEdges}).catch(()=>{});void reportFloatingSurfaces()})}catch{}try{await window.onMoved(event=>{floatingConfig.value.position={x:event.payload.x,y:event.payload.y};persistFloatingConfig();if(floatingSnapTimer)clearTimeout(floatingSnapTimer);floatingSnapTimer=globalThis.setTimeout(()=>void snapFloatingToMonitor(),120)})}catch{}}
...
async function syncFloatingData(){
 ...
 let success=false
 try{
   await syncReliabilitySource(undefined,true)
...
async function syncData(forceArena = false){
 if(isSyncing.value) return
 isSyncing.value=true
 let success=false
 try {
...
async function initializeMonitoring(){
 ...
 try{
  ...
  proxyEndpoints.value=await invoke<ProxyEndpoint[]>('start_all_proxies')
  allProxyStatus.value=clientIntegrationConnected.value
   ? `已自动恢复 ${proxyEndpoints.value.length} 条监控通道与调用工具连接`
   : `已自动恢复 ${proxyEndpoints.value.length} 条本机监控通道 · 可一键自动接入`
 }catch(error){
...
async function autoConnectEndpoint(endpoint:ProxyEndpoint){
 if(autoConnectBusy.value)return
 autoConnectBusy.value=true
 autoConnectStatus.value=`正在自动接入 ${endpoint.name}…`
 try{
  const result=await invoke<ClientIntegrationResult>('auto_connect_client',{localUrl:endpoint.local_url,anthropicUrl:endpoint.anthropic_url||null,accountName:endpoint.name})
...
async function autoConnectPreferred(){
 ...
 await autoConnectEndpoint(endpoint)
}
...
async function startAllProxies(){
 ...
 try{
  ...
  allProxyStatus.value=`代理已启动 · ${proxyEndpoints.value.length} 个账户 · 正在自动接入`
  await autoConnectPreferred()
  await syncData()
 }catch(error){
...
async function refreshSyncHealth(){
 try{
  ...
  if(opencode){
   opencodeLocalMode.value=opencode.mode
...
async function configureOpenCodeSource(){
 ...
 const config:SyncSourceConfig={id:'opencode-local',provider:'OpenCode Go',kind,mode:opencodeLocalMode.value,path:opencodeLocalPath.value,enabled:true,interval_seconds:30}
 const saved=await invoke<SyncSourceConfig>('configure_local_source',{config})
 opencodeLocalMode.value=saved.mode
...
async function syncReliabilitySource(sourceId?:string,silent=false){
 try{
  if(sourceId){
   await invoke<SyncHealth>('sync_source',{sourceId,force:!silent})
  }else{
   await invoke<SyncHealth[]>('sync_all_sources',{force:!silent})
  }
  usage.value=await invoke<Usage[]>('list_usage_v2',{days:floatingMode?7:30})
...
async function syncOpenCodeLocalUsage(silent=false){
 ...
 try{
  ...
  const result=await invoke<SyncHealth>('sync_source',{sourceId:'opencode-local',force:!silent})
  opencodeLocalStatus.value=result.last_error||`${result.detail} · ${result.latency_ms} ms`
  await refreshSyncHealth()
  if(!silent){usage.value=await invoke<Usage[]>('list_usage_v2',{days:30});successBurstKey.value++}
 }catch(error){
  if(!silent)opencodeLocalStatus.value=String(error)
...
function finishOnboarding(target:'finish'|'skip'|'accounts'){
 localStorage.setItem('token-manager-onboarding-v011','done')
 onboardingOpen.value=false
 if(target==='accounts')page.value='accounts'
}
function openConfig(p:Provider,account?:SavedAccount){configuring.value=p;editingAccountId.value=account?.id||'';configName.value=account?.name||`${p.name} 账户`;configUrl.value=account?.base_url||providerApiBases[p.name]||'';configKey.value='';configStatus.value=p.name==='OpenCode Go'?'粘贴 Go 订阅 API Key 后，将直接验证官方模型列表。':''}
...
async function sendAppPresence(event:'launch'|'heartbeat'){lastPresencePing=Date.now();try{await invoke('cloud_app_presence',{baseUrl:currentCloudBaseUrl(),appVersion:appVersion.value,event})}catch{/* 统计服务离线不得影响本地功能 */}}
...
<template>
 ...
 <InteractionEffects />
 <OnboardingFlow :open="onboardingOpen" :detected="{codex:Boolean(codexTokens||codexQuota),claude:Boolean(claudeUsage.length),opencode:syncHealthItems.some(item=>item.source_id==='opencode-local'&&item.status==='healthy')}" @finish="finishOnboarding('finish')" @skip="finishOnboarding('skip')" @open-accounts="finishOnboarding('accounts')" />
 ...
 <LiquidGlassEnvironment v-if="appTheme.material==='liquid'" :quality="glassQuality" :distortion="glassDistortion" :background-url="effectiveLiquidBackgroundImage" :video-path="liquidBackgroundVideoPath" />
  <EdgeScrollRail />
  <button class="health-launch health-launch-fixed" :class="{warning:syncHealthItems.some(item=>item.status==='error')}" @click="syncHealthOpen=true"><span><i></i><b>监控健康</b></span><small>{{syncHealthItems.filter(item=>item.status==='healthy').length}}/{{syncHealthItems.length||0}} 正常</small></button>
...
<header><div><h1>{{ {command:'AI 控制中心',dashboard:'模型用量',prompts:'Prompt 中心',arena:'Arena 排行榜',accounts:'账户与模型',reports:'报告中心',settings:'设置'}[page] }}</h1><p role="status" aria-live="polite">电脑时间 {{currentTimeText}} · 上次刷新 {{refreshedText}} · {{nextRefreshText}}</p></div><div class="header-actions"><SuccessBurst :trigger="successBurstKey" /><ThemeSwitcher /><button class="monitor-launch" :class="{running:proxyEndpoints.length,receiving:proxyRecentlyActive}" :disabled="autoConnectBusy" @click="monitorPrimaryAction"><ProxyGatewayIcon /><span class="monitor-launch-copy"><b>{{proxyRecentlyActive?'正在监控':autoConnectBusy?'正在自动接入':clientIntegrationConnected?'已自动接入':proxyEndpoints.length?'自动接入':'开启并自动接入'}}</b><small>{{proxyRecentlyActive?`${lastProxyTraffic?.provider} · ${lastProxyTraffic?.model}`:clientIntegrationConnected?'重启调用工具后自动统计':proxyEndpoints.length?'无需手动填写 Base URL':'本机转发 · 自动配置'}}</small></span></button><button class="sync" :class="{loading:isSyncing}" :disabled="isSyncing" @click="syncData(true)"><span class="sync-icon" aria-hidden="true">↻</span>{{isSyncing?'正在同步…':'同步数据'}}</button></div></header>
   <section v-if="page==='dashboard'||page==='accounts'" class="monitor-status-bar" :class="{'is-live':proxyRecentlyActive||ccSwitch.local_routing,'is-proxy-live':proxyRecentlyActive,'is-proxy-ready':proxyEndpoints.length&&!proxyRecentlyActive,'is-cc-live':ccSwitch.local_routing}" role="status" aria-live="polite">
    <div class="monitor-status-brand"><span class="monitor-status-icon"><ProxyGatewayIcon /></span><span><small>实时监控链路</small><b>{{proxyRecentlyActive?'API 用量正在实时接收':clientIntegrationConnected?'调用工具已自动接入':proxyEndpoints.length?'代理已启动，可自动接入':ccSwitch.local_routing?'CC Switch 路由已接管':'等待建立数据通道'}}</b><em>{{allProxyStatus||proxyConnectionDetail}}</em></span></div>
    ...
    <details v-if="proxyEndpoints.length" class="monitor-endpoints"><summary>自动接入账户与高级手动地址</summary><div><article v-for="endpoint in proxyEndpoints" :key="endpoint.local_url"><span><b>{{endpoint.name}}</b><small>OpenAI 兼容 · {{endpoint.account_id===autoConnectedAccountId?'当前自动接入':'可切换'}}</small><code>{{endpoint.local_url}}</code></span><button type="button" :disabled="autoConnectBusy" @click="autoConnectEndpoint(endpoint)">自动接入</button><button type="button" @click="copyProxyUrl(endpoint.local_url)">复制</button><template v-if="endpoint.anthropic_url"><span><small>Claude Code / Anthropic 兼容</small><code>{{endpoint.anthropic_url}}</code></span><button type="button" @click="copyProxyUrl(endpoint.anthropic_url)">复制</button></template></article></div></details>
   </section>
...
<div class="codex-title"><div><span>Codex · 本地深度监控</span><h2>直接读取客户端报告的额度状态</h2><p>优先使用 Codex 本地 rate_limits 事件；缺失的窗口才使用个人 Token 预算估算，全程不读取登录凭据。</p></div><button class="outline" @click="syncData()">刷新 Codex 数据</button></div>
     ...
     <div class="codex-quota-card card"><div><span class="quota-source" :class="{reported:codex5hIsReported}">{{codex5hIsReported?'客户端报告':'个人预算估算'}}</span><h2>5 小时滚动额度剩余</h2><strong><AnimatedNumber :value="remaining5h" suffix="%" /></strong><div class="track"><span :style="{width:used5hPercent+'%'}"></span></div><small>{{codex5hIsReported?'重置倒计时 '+codex5hResetText:`本地已用 ${used5h.toLocaleString()} / ${budget5h.toLocaleString()} Token`}}</small></div><div><span class="quota-source" :class="{reported:codex7dIsReported}">{{codex7dIsReported?'客户端报告':'个人预算估算'}}</span><h2>7 天滚动额度剩余</h2><strong><AnimatedNumber :value="remaining7d" suffix="%" /></strong><div class="track"><span :style="{width:used7dPercent+'%'}"></span></div><small>{{codex7dIsReported?'全额重置 ': '下一笔额度释放 '}}{{codex7dResetText}}</small></div></div>
    </section>
    <section v-else-if="selectedDashboard==='__claude__'" class="provider-overview card claude-overview"><div class="provider-overview-title"><ProviderMark name="Anthropic" /><div><span class="eyebrow">CLAUDE CODE · 本地专属仪表盘</span><h2>Claude Code 用量</h2><p>自动解析 ~/.claude/projects 中的 usage 元数据；不读取对话正文，每 30 秒更新。</p></div></div><div class="provider-overview-grid"><span><small>今日总 Token</small><b><AnimatedNumber :value="dashboardInput+dashboardOutput" /></b></span><span><small>缓存命中</small><b><AnimatedNumber :value="dashboardCached" /></b></span><span><small>今日请求</small><b><AnimatedNumber :value="dashboardToday.length" /></b></span><span><small>最近调用</small><b>{{dashboardLastUsage?new Date(dashboardLastUsage.at).toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'}):'暂无'}}</b></span></div></section>
    <section v-else-if="isApiDashboard" class="provider-overview card"><div class="provider-overview-title"><ProviderMark :name="selectedDashboardDef?.provider||''" /><div><span class="eyebrow">{{selectedDashboardDef?.provider}} · 独立仪表盘</span><h2>{{selectedDashboardDef?.model}}</h2><p>{{dashboardAccount?.name||'已配置账户'}} · 仅显示当前平台和模型，不混入 Codex 或其他模型。</p></div></div><div class="provider-overview-grid"><span><small>今日总 Token</small><b><AnimatedNumber :value="dashboardInput+dashboardOutput" /></b></span><span><small>缓存命中率</small><b><AnimatedNumber :value="dashboardCacheRate" suffix="%" /></b></span><span><small>今日请求</small><b><AnimatedNumber :value="dashboardToday.length" /></b></span><span><small>最近调用</small><b>{{dashboardLastUsage?new Date(dashboardLastUsage.at).toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'}):'暂无'}}</b></span></div></section>
...
<section v-if="isApiDashboard&&!dashboardUsage.length&&!dashboardBalance" class="card dashboard-empty" role="status"><span aria-hidden="true">◎</span><div><h2>尚未收到 {{selectedDashboardDef?.model}} 的调用数据</h2><p>点击“开启并自动接入”，Token Manager 会配置常见调用工具；重启正在运行的 Code、Cursor 或 Claude Code 后发起一次请求即可显示。</p><div class="empty-actions"><button @click="startAllProxies">开启并自动接入</button><button class="outline" :disabled="isSyncing" @click="syncData()">{{isSyncing?'正在同步…':'立即同步'}}</button></div></div></section>
    ...
    <template v-else-if="page==='accounts'">
     ...
     <div class="providers"><article class="card provider" v-for="p in accounts">
      <div class="provider-body"><div><div class="provider-head"><ProviderMark :name="p.name" /><h2>{{p.name}}</h2></div><p>{{p.description}}</p><p v-if="p.name==='OpenCode Go'" class="opencode-go-account-note">API Key 代理监控 + SQLite / JSON 本地双通道 · $12/5h · $30/周 · $60/月</p><span class="badge" :class="p.source">{{p.source}}</span></div>
      ...
      <div class="account-actions"><button class="outline" @click="openProviderPortal(p)">官网 ↗</button><button @click="openConfig(p)">＋ {{p.name==='OpenCode Go'?'添加 Go API Key':'添加账户'}}</button></div>
     </article></div>
    </template>
...
<template v-else><section class="card settings"><h2>悬浮窗与后台</h2><p>请在新的“设置 → 悬浮窗”二级页面中管理内置悬浮窗。</p><button @click="toggleFloating">{{floatingEnabled?'关闭悬浮窗':'一键启用悬浮窗'}}</button></section></template>
   <section v-if="page==='settings'" class="beginner-guide"><div class="guide-heading"><div><h2>新手配置教程</h2><p>照着下面三步操作，不需要理解代理原理。</p></div><span>{{savedAccounts.length?'账户已准备':'从第一步开始'}}</span></div><ol><li><b>添加 API 账户</b><p>进入“账户与模型”，选择平台，点击“添加账户”，粘贴官网创建的 API Key。密钥只会以 Windows DPAPI 密文保存在本机。</p><button class="outline" @click="page='accounts'">前往添加账户</button></li><li><b>一键开启并自动接入</b><p>软件会启动本机代理，并自动配置常见 Code、SDK 与 Claude Code。已经打开的调用工具只需重启一次。</p><button @click="startAllProxies">开启并自动接入</button><button class="outline" :disabled="autoConnectBusy" @click="restoreClientConnection">恢复原连接</button><small>{{autoConnectStatus||'修改前会自动备份原始连接配置。'}}</small></li><li><b>选择仪表盘</b><p>返回仪表盘，打开“全部模型”查看总览，打开“Codex 专属”查看本地额度，或添加单个 API 模型仪表盘查看详细调用。</p><button class="outline" @click="page='dashboard';selectDashboard('__all__')">查看全部模型</button></li></ol><div class="codex-guide-note"><b>Codex 不需要代理</b><span>Codex 专属仪表盘会直接读取本机日志；API 平台调用则由自动接入的本机代理统计。</span></div></section>
...
.health-launch-fixed{position:fixed;z-index:45;top:66px;right:26px}.health-launch{display:flex;align-items:center;gap:10px;padding:8px 11px;border:1px solid var(--tm-line);border-radius:13px;background:color-mix(in srgb,var(--tm-bg) 94%,transparent);color:var(--tm-ink);box-shadow:0 8px 24px rgba(0,0,0,.06);backdrop-filter:blur(16px)}.health-launch>span{display:flex;align-items:center;gap:6px}.health-launch i{width:7px;height:7px;border-radius:50%;background:#34c759;box-shadow:0 0 0 4px color-mix(in srgb,#34c759 13%,transparent)}.health-launch b{font-size:10px}.health-launch small{color:var(--tm-muted);font-size:8px}.health-launch.warning i{background:#ff9f0a;box-shadow:0 0 0 4px color-mix(in srgb,#ff9f0a 14%,transparent)}
...
@media(max-width:860px){.health-launch-fixed{top:64px;right:16px}.health-launch small{display:none}}
</style>
