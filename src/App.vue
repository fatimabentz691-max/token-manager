...
const modelDashboards = ref<ModelDashboard[]>(Array.isArray(storedDashboards)?storedDashboards.map((item:Partial<ModelDashboard>)=>({
 key:item.key||'',kind:item.kind||'account_model',accountId:item.accountId||'',sourceId:item.sourceId||'',agentId:item.agentId||'',provider:item.provider||'',model:item.model||'',label:(item.kind==='local_agent'?(item.label||'').replace(/ · 本地总览$/,''):(item.label||item.model||'模型仪表盘'))
   ...
   if(event.payload.status==='completed'){
     proxyEndpoints.value=event.payload.endpoints
      if(event.payload.agent_ids.length&&event.payload.stage==='ready'&&event.payload.endpoints[0]){
       // 多 Agent 一键接入时优先记录 DeepSeek Harness 与 DeepSeek 账户的配对，保证合并用量展示正确
       const harnessIndex=event.payload.agent_ids.indexOf('deepseek-harness')
       const agentIndex=harnessIndex>=0?harnessIndex:0
       const dsEndpoint=event.payload.endpoints.find(item=>item.provider==='DeepSeek')||event.payload.endpoints[0]
       autoConnectedAccountId.value=harnessIndex>=0?dsEndpoint.account_id:event.payload.endpoints[0].account_id
       autoConnectedAgentId.value=event.payload.agent_ids[agentIndex]
...
const dashboardTitle=computed(()=>{
  if(selectedDashboard.value==='__all__')return '全部模型'
  if(selectedDashboard.value==='__codex__')return 'Codex 专属'
  if(selectedDashboard.value==='__claude__')return 'Claude Code 专属'
  const target=selectedDashboardDef.value
  if(!target)return '模型仪表盘'
  // 仪表盘标题只显示名字：本地 Agent 总览用 Agent 名称，不再附加“本地总览”等功能描述。
  if(target.kind==='local_agent')return localAgents.value.find(item=>item.id===target.agentId)?.name||target.label.replace(/ · 本地总览$/,'')||'本地 Agent'
  return target.label||'模型仪表盘'
})
...
const dashboardAccount=computed(()=>{
  const target=selectedDashboardDef.value
  if(!target)return undefined
  if(target.kind==='account_model')return savedAccounts.value.find(item=>item.id===target.accountId)
  // 本地 Agent 仪表盘只关联用户明确接入本 Agent 的账户；未接入时仅当本机只有一个 DeepSeek 账户才兜底，
  // 避免把其他账户的余额混入当前 Agent 页面。
  if(target.provider==='DeepSeek'){
    const connected=savedAccounts.value.find(item=>item.id===autoConnectedAccountId.value)
    if(connected)return connected
    const deepseekAccounts=savedAccounts.value.filter(item=>item.provider==='DeepSeek')
    if(deepseekAccounts.length===1)return deepseekAccounts[0]
  }
  return undefined
})
...
const availableAgentDashboards=computed<ModelDashboard[]>(()=>localAgents.value.flatMap(agent=>{
 ...
 const overview:ModelDashboard={key:`agent:${source.id}:*`,kind:'local_agent',accountId:'',sourceId:source.id,agentId:agent.id,provider:agent.provider,model:'全部模型',label:agent.name}
...
async function monitorPrimaryAction(){
 ...
 if(!proxyRecentlyActive.value){await connectAllDetectedAgents();return}
 await syncData()
}
/** 一键接入：为每个已检测且协议兼容的本地 Agent 接入对应账户端点。 */
async function connectAllDetectedAgents(){
 const candidates=localAgents.value.filter(agent=>agent.detected&&agent.proxy_protocol)
 if(!candidates.length){allProxyStatus.value='未检测到可自动接入的本地 Agent · 请到“账户与模型”确认检测结果';return}
 const connected:string[]=[];const manual:string[]=[]
 for(const agent of candidates){
  const endpoint=proxyEndpoints.value.find(item=>agent.id==='claude-code'?Boolean(item.anthropic_url):agent.id==='deepseek-harness'?item.provider==='DeepSeek':false)
  if(endpoint&&(agent.id==='deepseek-harness'||agent.id==='claude-code')){await autoConnectEndpoint(endpoint,agent.id);connected.push(agent.name)}
  else if(agent.proxy_protocol)manual.push(agent.name)
 }
 allProxyStatus.value=connected.length?`已一键接入 ${connected.join('、')}`:'未发现可自动接入的 Agent'
 if(manual.length)allProxyStatus.value+=` · ${manual.join('、')} 请在卡片中复制 Base URL 手动接入`
}
async function startAllProxies(requestedAgentId?:string){
 ...
 try{
  await refreshAccounts()
  // 一键接入：先自动识别本机 Agent，再启动全部账户代理并接入全部协议兼容的已检测 Agent
  await refreshSyncHealth()
  if(!savedAccounts.value.length)throw new Error('请先添加一个 API 账户')
  const accountIds=savedAccounts.value.map(item=>item.id)
  const detectedAgents=localAgents.value.filter(agent=>agent.detected&&agent.proxy_protocol)
  const agentIds=requestedAgentId?[requestedAgentId]:detectedAgents.map(agent=>agent.id)
  const job=await invoke<ProxyJob>('start_proxy_group',{accountIds,agentIds})
...
<ArenaCenter v-if="page==='arena'" ref="arenaCenter" />
 ...
 <section class="dashboard-context-heading" data-liquid-ignore><div><img v-if="selectedDashboard==='__codex__'" class="codex-official-mark" :src="codexOfficialLogo" alt="Codex 官方标志" /><ProviderMark v-else-if="isClaudeDashboard" name="Anthropic" /><ProviderMark v-else-if="isApiDashboard" :name="selectedDashboardDef?.provider||''" /><div><SupplementalHelp help-id="dashboard-heading-detail" detail="图表优先展示；下方再呈现 Token、请求、缓存、余额和来源说明。"><h2>{{dashboardTitle}}</h2></SupplementalHelp></div></div><span>{{selectedDashboard==='__codex__'?codexSeries.length:dashboardUsage.length}} 条真实记录</span></section>
     ...
     <div v-if="selectedDashboard==='__all__'" class="dashboard-heading"><div><h2>{{dashboardTitle}}</h2></div><span>{{dashboardUsage.length}} 条真实调用</span></div>
...
<div class="codex-title"><div><SupplementalHelp help-id="codex-title-detail" kind="source" detail="优先使用 Codex 本地 rate_limits 事件；缺失的窗口才使用个人 Token 预算估算，全程不读取登录凭据。"><h2>Codex 额度状态</h2></SupplementalHelp></div><button class="outline" @click="syncData()">刷新 Codex 数据</button></div>
    ...
    <section v-if="isOpenCodeGoDashboard" class="opencode-go-monitor">
     <div class="opencode-go-heading card">
      <div class="provider-overview-title"><ProviderMark name="OpenCode Go" /><div><h2>套餐额度</h2></div></div>
