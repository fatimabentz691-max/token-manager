...
fn response_usage(body: &[u8], model: &str, provider: &str, status: u16) {
    ...
    let _ = db.execute(
        "INSERT INTO usage_events VALUES(?1,?2,?3,?4,?5,?6,?7,?8,?9)",
        params![
            uuid_like(),
            provider,
            model,
...
async fn proxy_request(
    ...
    let mut remote = match outbound.send().await {
        ...
        Err(error) => {
            ...
            runtime
                .app
                .emit("usage-updated", runtime.provider.clone())
                .ok();
    ...
    if is_event_stream {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            stream_runtime
                .app
                .emit("usage-updated", stream_runtime.provider.clone())
                .ok();
        });
    ...
    response_usage(&bytes, &model, &runtime.provider, status.as_u16());
    if runtime.provider == "DeepSeek" {
...
fn save_usage(event: UsageEvent, state: State<AppDb>) -> Result<(), String> {
    let db = state.0.lock().map_err(|_| "数据库锁定")?;
    db.execute(
        "INSERT OR REPLACE INTO usage_events VALUES(?1,?2,?3,?4,?5,?6,?7,?8,?9)",
        params![
            event.id,
            event.provider,
            event.model,
...
fn export_usage_xlsx(
    ...
    let db = state.0.lock().map_err(|_| "数据库锁定")?;
    let mut statement=db.prepare("SELECT id,provider,model,at,input_tokens,output_tokens,cached_tokens,cost,task FROM usage_events ORDER BY at DESC").map_err(|e|e.to_string())?;
